...
export default function TestStreamPage() {
  ...
  // Initialize MediaRecorder when stream arrives
  useEffect(() => {
    if (localStream) {
  ...
  // Connect transcript WS when WebRTC is live
  useEffect(() => {
  ...
  // Disconnect everything without upload
  ...
  // Finish consultation: stop, upload, finish endpoint, navigate
  const handleFinishConversation = async () => {
    try {
      setIsFinishing(true);

      // 1) Stop WS and WebRTC
      disconnectTranscript();
      disconnect();

      // 2) Stop recorder and wait
      await stopRecording();
      ...
      const filepath = filename; // Root of "videos" bucket
      ...
      console.log("🎥 Video uploaded to videos/" + filepath);

      // 5) Call finish endpoint (optionally include file URL)
      const finRes = await fetch(
        "http://localhost:8000/api/stream/finish/default",
        { method: "POST", headers: { "Content-Type": "application/json" } }
      );
      if (!finRes.ok) throw new Error(`Finish API failed: ${finRes.status}`);
      await finRes.json();
    ...
    <div className="min-h-screen bg-primary-background">
      <div className="h-screen flex flex-col">
        ...
        <div className="flex-shrink-0 p-4 border-b bg-white/80 backdrop-blur-sm">
          <h1 className="text-2xl font-bold text-center">Live Medical Consultation</h1>
        </div>

        {/* Status Bar */}
        <div className="flex-shrink-0 p-3 bg-white/60 backdrop-blur-sm border-b">
          <div className="flex items-center justify-center gap-8">
            <StatusDot on={!!localStream} label="Media" />
            <StatusDot on={isConnected} label="AI" />
            <StatusDot on={isTranscriptConnected} label="Transcript" />
            <div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 p-4">
            <div className="h-full bg-white/80 backdrop-blur-sm rounded-xl border p-4 flex flex-col">
              <PaneHeader icon={<Video />} title="Live Video" />
              <div className="flex-1 bg-black rounded-lg overflow-hidden relative">
                <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover" />
                {!localStream && (
                  <Overlay message="Camera access required" icon={<Video />} />
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 p-4 flex flex-col">
            <div className="bg-white/80 backdrop-blur-sm rounded-xl border p-4 flex flex-col h-full">
              <PaneHeader icon={<MessageSquare />} title="Live Conversation" />
              <div className="flex-1 overflow-y-auto bg-gray-50 rounded-lg p-3 mb-3">
                {transcripts.length === 0 ? (
                  <Overlay message="Ready to consult" icon={<MessageSquare />} />
                ) : (
                  transcripts.map((t) => (
                    <MessageBubble key={t.id} transcript={t} />
                  ))
                )}
        ...
        <div className="flex-shrink-0 p-4 bg-white/80 backdrop-blur-sm border-t">
          <div className="flex items-center justify-center gap-4">
            ...
            >
              {isConnected ? <CheckCircle /> : <Mic />} {isConnected ? "Connected" : "Start Consultation"}
            </button>
            <button
            ...
            >
              {isFinishing ? <Loader2 className="animate-spin" /> : <MessageSquare />} Finish & Analyze
            </button>
            <button
...
function StatusDot({ on, label }: { on: boolean; label: string }) {
  return (
    <div className="flex items-center gap-2">
      <div className={`w-3 h-3 rounded-full ${on ? "bg-green-500" : "bg-red-500"}`} />
...
function Overlay({ message, icon }: { message: string; icon: React.ReactNode }) {
  return (
    <div className="absolute inset-0 flex items-center justify-center text-center text-white">
      <div className="p-4 bg-white/20 rounded-2xl backdrop-blur-sm inline-block mb-2">
        {icon}
      </div>
...
function MessageBubble({ transcript }: { transcript: any }) {
  const isUser = transcript.role === "user";
  return (
    <div className={`p-2 rounded-lg max-w-[90%] text-sm mb-2 ${isUser ? "bg-blue-600 text-white ml-auto" : "bg-gray-200 text-gray-900 mr-auto"}`}>
      <div className="font-medium text-xs mb-1 opacity-90">{isUser ? "You" : "AI"}</div>
...
function Instructions() {
  return (
    <div className="bg-gray-100 border border-gray-200 rounded-lg p-2 text-xs">
      <div className="font-semibold mb-1">How it works:</div>
      <ol className="list-decimal ml-4 space-y-0.5">
        <li>Start consultation</li>
